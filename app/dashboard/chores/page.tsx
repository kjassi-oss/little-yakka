              ...
              return (
                <button key={child.id} onClick={() => setFilterChildId(sel ? null : child.id)}
                  ...
                  {child.avatar_url
                    ...
                    : <div className="w-14 h-14 rounded-full flex items-center justify-center text-[36px] leading-none overflow-hidden bg-white"
              ...
              <div className="grid grid-cols-7 gap-1.5 p-2 bg-gray-50 rounded-2xl">
                {(emojiSearch.trim()
                  ...
                  <button key={`${e}-${i}`} onClick={() => setEmoji(e)}
                    ...
                    <span className={`text-2xl leading-none p-1.5 rounded-xl ${emoji === e ? 'ring-2 ring-purple-400 bg-white' : 'bg-white/60'}`}>{e}</span>
                      ...
                      <div className="flex -space-x-1.5 justify-center">
                        {assignedKids.map(child => (
                          child.avatar_url
                            ...
                            : <div key={child.id} className="w-6 h-6 rounded-full flex items-center justify-center text-[15px] leading-none overflow-hidden bg-white"
                                style={{ border: `1.5px solid ${child.colour}` }}>{child.avatar}</div>
                        ))}
                      </div>
                    </div>
                  )
                ...
                <div key={child.id} className="bg-white rounded-3xl shadow-sm overflow-hidden">
                  <button onClick={() => setExpandedDoneChild(open ? null : child.id)}
                    ...
                    {child.avatar_url
                      ...
                      : <div className="w-9 h-9 rounded-full flex items-center justify-center text-[24px] leading-none overflow-hidden bg-white flex-shrink-0"
